...
for line in file_lines:
    if line[YEAR] == user_year:
        if line[TEAM] == user_team or lines[NOC] == user_team:
...
bronze_conter = 0
#dictionar
for j, i in zip(range(10), athletes_dict):
    print(f'{j+1}. {i} : {athletes_dict[i]['discipline']} : {athletes_dict[i]['medal']}')
